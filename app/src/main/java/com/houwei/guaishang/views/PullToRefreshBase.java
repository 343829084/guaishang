...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private PullToRefreshLoadingLayout mHeaderLayout;
	private PullToRefreshLoadingLayout mFooterLayout;
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		if (mRefreshableView instanceof AbsListView) {
			AbsListView listView = (AbsListView)mRefreshableView;
			if (isRefreshing() && (listView.getAdapter() == null || listView.getAdapter().isEmpty())) {
				return true;
			}
		}
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				scrollTo(0,mCurrentMode == MODE_PULL_DOWN_TO_REFRESH ? -mHeaderHeight : mHeaderHeight);
			} else {
				scrollTo(0,0);
			}
		}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.AppStorePullToRefresh_AppStoreptrMode)) {
			mMode = a.getInteger(R.styleable.AppStorePullToRefresh_AppStoreptrMode, MODE_PULL_DOWN_TO_REFRESH);
		}
		...
		if (mMode == MODE_PULL_DOWN_TO_REFRESH || mMode == MODE_BOTH) {
			mHeaderLayout = new PullToRefreshLoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseToRefreshLabel, pullToRefreshLabel, refreshingLabel, a, 0);
		...
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
			mFooterLayout = new PullToRefreshLoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseToLoadLabel, pullToLoadLabel, loadingLabel, a, 1);
		...
		// Styleables from XML
		if (a.hasValue(R.styleable.AppStorePullToRefresh_AppStoreptrHeaderBackground)) {
			setBackgroundResource(a.getResourceId(R.styleable.AppStorePullToRefresh_AppStoreptrHeaderBackground, Color.WHITE));
		}
		if (a.hasValue(R.styleable.AppStorePullToRefresh_AppStoreptrAdapterViewBackground)) {
			mRefreshableView.setBackgroundResource(a.getResourceId(R.styleable.AppStorePullToRefresh_AppStoreptrAdapterViewBackground, Color.WHITE));
		}
		a.recycle();
		a = null;
